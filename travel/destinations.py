...
from datetime import datetime  # Import datetime for strftime
...
def show(id):
    ...
    # Fetch all comments related to the destination
    comments = db.session.scalars(db.select(Comment).where(Comment.destination_id == id)).all()
    
    # Format the created_at date for each comment
    formatted_comments = [
        {
            'text': comment.text,
            'user': comment.user,
    ...
    return render_template('destinations/show.html', destination=destination, form=form, comments=formatted_comments)
